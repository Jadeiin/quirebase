...
<div class="grid items-start gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(17rem,0.5fr)]">
	<div class="stack">
		...
		>
			...
			<div class="bg-surface/55 grid grid-cols-4 border-t border-surface-300-700">
				<div class="grid gap-0.5 border-r border-surface-300-700 p-4">
	...
	<aside class="grid content-start gap-5">
		{#if data.thumbnail && !imageLoadFailed}
			<section
				class="overflow-hidden rounded-xl border border-surface-300-700 bg-surface-50-950 p-2 shadow-sm"
			>
				<a
		...
		<section class="rounded-xl border border-surface-300-700 bg-surface-50-950 p-5 shadow-sm">
			...
			<div class="grid gap-2">
				{#each details?.metadata.urls ?? [] as url, index (url)}
					<a
						class="grid min-w-0 gap-0.5 rounded-lg border border-surface-300-700 px-3 py-2 text-sm no-underline hover:bg-primary-50-950"
						href={url}
						target="_blank"
