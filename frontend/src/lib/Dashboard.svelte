...
{#if dashboard.isPending}<div class="grid min-h-56 place-items-center text-surface-600-400">
		{$t('Loading dashboard…')}
	</div>
{:else if dashboard.isError}<div class="grid min-h-56 place-items-center text-error-700-300">
		{$t('Unable to load the dashboard.')}
	</div>
{:else}
	<div class="mb-5 grid gap-3 sm:grid-cols-3">
		<a
			class="flex items-center gap-3 rounded-xl border border-surface-300-700 bg-surface-50-950 p-4 no-underline shadow-sm transition-colors hover:border-surface-400-600"
			href={resolve('/library')}
		>
			<span
				class="grid size-10 place-items-center rounded-lg bg-primary-50-950 text-primary-800-200"
	...
	<div class="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)]">
		<div class="grid content-start gap-5">
			<section
				class="overflow-hidden rounded-xl border border-surface-300-700 bg-surface-50-950 shadow-sm"
			>
				...
				<div class="divide-line divide-y">
					{#each dashboard.data?.recent_items ?? [] as recent (recent.item.id)}
						<a
							class="group grid gap-1 px-5 py-4 no-underline transition-colors hover:bg-primary-50-950/60"
					...
					{:else}
						<div
							class="grid min-h-40 place-items-center p-6 text-center text-sm text-surface-600-400"
						>
			...
			>
				<header class="flex items-center justify-between border-b border-surface-300-700 px-5 py-4">
					...
					<a
						...
						href={resolve('/import')}>{$t('Import more')}</a
					>
				</header>
				<div class="divide-line divide-y">
					{#each (dashboard.data?.new_items ?? []).slice(0, 5) as item (item.id)}
						<a
							class="group grid gap-1 px-5 py-3.5 no-underline hover:bg-primary-50-950/60"
		...
		<div class="grid content-start gap-5">
			<section class="rounded-xl border border-surface-300-700 bg-surface-50-950 p-5 shadow-sm">
				<div class="mb-4">
					<p class="m-0 text-xs font-bold tracking-wide text-primary-700-300 uppercase">
						{$t('Get started')}
					</p>
				...
				<div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
					{#each [{ href: '/import' as const, icon: 'import', title: $t('Import Items'), detail: $t('DOI, files, or PDFs') }, { href: '/discovery' as const, icon: 'search', title: $t('Discover'), detail: $t('Search scholarly sources') }, { href: '/projects' as const, icon: 'projects', title: $t('New Project'), detail: $t('Organize a collection') }, { href: '/library' as const, icon: 'library', title: $t('Search Library'), detail: $t('Find saved research') }] as action (action.href)}
						<a
							class="group grid gap-2 rounded-lg border border-surface-300-700 p-3 no-underline hover:border-primary-700-300/40 hover:bg-primary-50-950"
			...
			<section class="rounded-xl border border-surface-300-700 bg-surface-50-950 p-5 shadow-sm">
				<div class="mb-4 flex items-center justify-between">
					...
					<a
						...
						href={resolve('/projects')}>{$t('View all')}</a
					>
				</div>
			...
			<section class="rounded-xl border border-surface-300-700 bg-primary-50-950 p-5">
				...
				<a class="text-sm font-semibold text-primary-800-200 no-underline" href={resolve('/tools')}
