<script lang="ts">
	...
	import { itemDetailsQuery, itemFilesQuery, itemWorkspaceQuery } from '$lib/features/item/queries';
	...
	const files = createQuery(() => itemFilesQuery(params.itemId, true));
	...
	const fileUpload = createMutation(() =>
		fileUploadMutationOptions(params.itemId, queryClient, trackDocument)
	);
	const fileRemoteUpload = createMutation(() =>
		fileRemoteUploadMutationOptions(params.itemId, queryClient, trackDocument)
	);
	const fileDelete = createMutation(() => fileDeleteMutationOptions(params.itemId, queryClient));
