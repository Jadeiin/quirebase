...
Status: accepted for the first complete rewrite release.

OCR, OIDC, email, a public REST API, MCP, AI features and a third-party plugin SDK are not required for parity with the independently defined MVP. Adding them now would introduce external process execution, credential storage and long-lived compatibility contracts before the core domain is stable.
